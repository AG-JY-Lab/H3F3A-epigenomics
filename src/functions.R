STARo2df<-function(sample_names, samples_path, rename_colnames = TRUE, preserve_config_ordering = TRUE,
  ...
  for (sample_path in star_counts_full_paths){
    for (sample_name in sample_names){
      if (grepl(sample_name, sample_path)){
        if (rename_colnames){
          names(sample_path) <- names(sample_names[which(sample_name == sample_names)])
        }
        else{
  ...
  if (preserve_config_ordering){
    # This preserves the ordering of columns to match the order in the config.yml.
    sample_names_vector <- names(sample_names)
    column_ordering <- c(gene_naming, sample_names_vector)
    print(column_ordering)
    STAR_samples_merged_reordered <- STAR_samples_merged[, column_ordering]
    
    return(STAR_samples_merged_reordered)
  }
